...
pub fn EdgePolicyEditor(
    ...
    /// Pinned summaries available for catch-up rules.
    summaries: Signal<Vec<(crate::models::MessageId, String)>>,
    ...
    view! {
        <div class="edge-policy-editor flex h-full" style=match layout_mode {
            ...
            <div style=match layout_mode {
                ...
                <EdgeList
                    ...
                    on_toggle=move |id, enabled| {
                        if let Some(mut edge) = edges.get_untracked().into_iter().find(|edge| edge.id == id) {
            ...
            // Zone 2: Detail panel
            <div class="flex-1 overflow-y-auto p-5" style=match layout_mode {
                LayoutMode::Sidebar => "",
                LayoutMode::FullTab => "min-width: 300px;",
            }>
                {move || match selected_edge.get() {
                    Some(edge) => view! {
                        <EdgeDetailPanel edge=edge summaries=summaries on_save=on_update />
                    }.into_any(),
                    None => view! {
