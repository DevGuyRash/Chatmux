//! Pure ChatGPT DOM classification and identity rules.
...
pub(super) fn conversation_id_from_path(path: &str) -> Option<String> {
    segment_after(path, "c")
}

pub(super) fn classify_role(author_role: Option<&str>) -> Option<MessageRole> {
...
mod tests {
    ...
    #[test]
    fn parses_chatgpt_conversation_route() {
        assert_eq!(
    ...
    #[test]
    fn classifies_chatgpt_roles_from_owned_attribute() {
        assert_eq!(classify_role(Some("user")), Some(MessageRole::User));
        assert_eq!(
            classify_role(Some("assistant")),
            Some(MessageRole::Assistant)
        );
        assert_eq!(classify_role(Some("tool")), None);
    }
    ...
    fn rate_limit_classification_requires_blocking_language() {
        assert!(text_indicates_rate_limit("You've reached the rate limit"));
        assert!(!text_indicates_rate_limit("Temporary warning"));
    }
}
