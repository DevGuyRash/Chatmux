//! Pure Claude DOM classification and identity rules.
...
pub(super) fn classify_role(
    ...
    .to_ascii_lowercase();
    if semantic_text.contains("user-message") {
        Some(MessageRole::User)
    } else if semantic_text.contains("assistant-message")
        || semantic_text.contains("claude-response")
    {
...
mod tests {
    ...
    #[test]
    fn parses_claude_conversation_route() {
        assert_eq!(
            conversation_id_from_path("/chat/abc-123"),
            Some("abc-123".to_owned())
        );
        assert_eq!(conversation_id_from_path("/new"), None);
    }
    ...
    fn classifies_provider_message_roles() {
        ...
        assert_eq!(
            classify_role(None, None, Some("font-claude-response")),
            Some(MessageRole::Assistant)
        );
    ...
    fn rate_limit_classification_requires_blocking_language() {
        ...
        assert!(!text_indicates_rate_limit("Claude saved your draft"));
    }
}
